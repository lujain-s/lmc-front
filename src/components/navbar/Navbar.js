...
import { FaHome, FaInfoCircle, FaPhone, FaConciergeBell, FaSignInAlt, FaSignOutAlt } from 'react-icons/fa';
import './Navbar.css';
            ...
            <ul className="navbar-nav ms-auto">
              {menuItems.map(({ path, icon, label }, index) => (
                <li className="nav-item" key={index}>
                  <Link to={path} className="nav-link d-flex align-items-center">
              ...
              : <li className="nav-item" key={5}>
              <Link onClick={logout} className="nav-link d-flex align-items-center">
